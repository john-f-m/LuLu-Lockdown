...
static NSString *getPendingConnectionsPath(void) {
  return [INSTALL_DIRECTORY
      stringByAppendingPathComponent:PENDING_CONNECTIONS_FILE];
}
...
static NSMutableDictionary *makeConnectionEntry(NEFilterSocketFlow *flow,
                                                Process *process,
                                                NSString *decision,
                                                NSString *reason) {
  // entry
...
static void appendPendingConnection(NSDictionary *entry) {
  @synchronized(NSFileManager.defaultManager) {
    ...
    for (NSDictionary *existing in pending) {
      if ((NSOrderedSame ==
           [existing[KEY_PATH] caseInsensitiveCompare:entry[KEY_PATH]]) &&
          (NSOrderedSame ==
           [existing[KEY_ENDPOINT_ADDR]
               caseInsensitiveCompare:entry[KEY_ENDPOINT_ADDR]]) &&
          (NSOrderedSame ==
           [existing[KEY_ENDPOINT_PORT]
               caseInsensitiveCompare:entry[KEY_ENDPOINT_PORT]]) &&
          ([existing[KEY_PROTOCOL] intValue] ==
           [entry[KEY_PROTOCOL] intValue])) {
        return;
      }
    }
...
- (void)startFilterWithCompletionHandler:
  ...
  // init meta list
  metaList = [[BlockOrAllowList alloc] init:@""];

  // load domains
  [metaList addFromFile:[INSTALL_DIRECTORY
                            stringByAppendingPathComponent:
                                @"MetaBlock/generated/meta_domains.txt"]];

  // load prefixes
  [metaList addFromFile:[INSTALL_DIRECTORY
                            stringByAppendingPathComponent:
                                @"MetaBlock/generated/meta_ipv4_prefixes.txt"]];
  [metaList addFromFile:[INSTALL_DIRECTORY
                            stringByAppendingPathComponent:
  ...
  filterRule =
      [[NEFilterRule alloc] initWithNetworkRule:networkRule
                                         action:NEFilterActionFilterData];

  // init filter settings
  filterSettings = [[NEFilterSettings alloc] initWithRules:@[ filterRule ]
                                             defaultAction:NEFilterActionAllow];

  // apply rules
  [self applySettings:filterSettings
      completionHandler:^(NSError *_Nullable error) {
        ...
        // error?
        if (nil != error)
          os_log_error(logHandle, "ERROR: failed to apply filter settings: %@",
...
- (void)stopFilterWithReason:(NEProviderStopReason)reason
  ...
  // log msg
  os_log_debug(logHandle, "method '%s' invoked with %ld", __PRETTY_FUNCTION__,
               (long)reason);
...
- (NEFilterNewFlowVerdict *)handleNewFlow:(NEFilterFlow *)flow {
  ...
  // log msg
  os_log_debug(logHandle, "remote endpoint: %{public}@ / url: %{public}@",
               remoteEndpoint, flow.URL);

  // ignore non-outbound traffic
  //  even though we init'd `NETrafficDirectionOutbound`, sometimes get inbound
  //  traffic :|
  if (NETrafficDirectionOutbound != socketFlow.direction) {
    // log msg
    os_log_debug(logHandle, "ignoring non-outbound traffic (direction: %ld)",
                 (long)socketFlow.direction);
...
- (NEFilterNewFlowVerdict *)processEvent:(NEFilterFlow *)flow {
  ...
  // in cache
    ...
    os_log_debug(logHandle,
                 "found process object in cache: %{public}@ (pid: %d)",
  ...
  //  process exited? deny
  pid_t pid =
      audit_token_to_pid(*(audit_token_t *)flow.sourceAppAuditToken.bytes);
  if (!isAlive(pid)) {
    // dbg msg
    ...
    appendConnectionEvent(makeConnectionEntry(
        (NEFilterSocketFlow *)flow, process, @"block", @"process_exited"));
    goto bail;
  }
  ...
  if (nil == process) {
    ...
    os_log_error(logHandle,
                 "ERROR: failed to create process for flow, will allow");

    // log
    appendConnectionEvent(makeConnectionEntry(
        (NEFilterSocketFlow *)flow, process, @"allow", @"process_unavailable"));
  ...
  if ((YES == [preferences.preferences[PREF_META_BLOCK] boolValue]) &&
      (YES == [metaList isMatch:(NEFilterSocketFlow *)flow])) {
    // dbg msg
    os_log_debug(logHandle, "flow matched meta block list, DENYING");
  ...
  postNotificationName:LULU_EVENT object:@"new flow" userInfo:[alerts
  create:(NEFilterSocketFlow*)flow process:process]
  ...
  */
  ...
  if ((nil != consoleUser) && (nil != alerts.consoleUser) &&
    ...
    // dbg msg
    os_log_debug(logHandle,
                 "current console user '%{public}@', is different than "
                 "'%{public}@', so allowing flow: %{public}@",
                 consoleUser, alerts.consoleUser,
                 ((NEFilterSocketFlow *)flow).remoteEndpoint);
  ...
  if (YES == [preferences.preferences[PREF_BLOCK_MODE] boolValue]) {
    ...
    if ((YES == [preferences.preferences[PREF_USE_ALLOW_LIST] boolValue]) &&
      ...
      // dbg msg
      os_log_debug(logHandle, "client in block mode, but flow matches item in "
                              "allow list, so allowing");
    ...
    // dbg msg
    os_log_debug(logHandle,
                 "client in block mode (and item not on allow list), so "
                 "disallowing %d/%{public}@",
                 process.pid, process.binary.name);
    ...
    // log
    appendConnectionEvent(makeConnectionEntry(
        (NEFilterSocketFlow *)flow, process, @"block", @"block_mode"));
  ...
  if ((YES == [preferences.preferences[PREF_USE_BLOCK_LIST] boolValue]) &&
    ...
    // dbg msg
    os_log_debug(logHandle,
                 "client is using block list '%{public}@' (%lu items) ...will "
                 "check for match",
                 preferences.preferences[PREF_BLOCK_LIST],
                 (unsigned long)blockList.items.count);
    ...
    if (YES == [blockList isMatch:(NEFilterSocketFlow *)flow]) {
      ...
      // log
      appendConnectionEvent(makeConnectionEntry(
          (NEFilterSocketFlow *)flow, process, @"block", @"block_list"));
    ...
    // dbg msg
      ...
      os_log_debug(logHandle, "remote endpoint/URL not on block list...");
  }
  ...
  if ((YES == [preferences.preferences[PREF_USE_ALLOW_LIST] boolValue]) &&
    ...
    // dbg msg
    os_log_debug(logHandle,
                 "client is using allow list '%{public}@' (%lu items) ...will "
                 "check for match",
                 preferences.preferences[PREF_ALLOW_LIST],
                 (unsigned long)allowList.items.count);
    ...
    if (YES == [allowList isMatch:(NEFilterSocketFlow *)flow]) {
      ...
      // log
      appendConnectionEvent(makeConnectionEntry(
          (NEFilterSocketFlow *)flow, process, @"allow", @"allow_list"));
    ...
    // dbg msg
      ...
      os_log_debug(logHandle, "remote endpoint/URL not on allow list...");
  }
  ...
  matchingRule = [rules find:process
                        flow:(NEFilterSocketFlow *)flow
                    csChange:&csChange];
  if (nil != matchingRule) {
    // dbg msg
    os_log_debug(logHandle, "found matching rule for %d/%{public}@: %{public}@",
                 process.pid, process.binary.name, matchingRule);
    ...
    if (RULE_STATE_BLOCK == matchingRule.action.intValue) {
      ...
      // log
      appendConnectionEvent(makeConnectionEntry(
          (NEFilterSocketFlow *)flow, process, @"block", @"matching_rule"));
    }
    // allow (msg)
    else {
      os_log_debug(logHandle, "rule says: ALLOW");
      appendConnectionEvent(makeConnectionEntry(
          (NEFilterSocketFlow *)flow, process, @"allow", @"matching_rule"));
    }
  ...
  if (YES == csChange) {
    // dbg msg
    os_log_debug(logHandle,
                 "found rule set for %d/%{public}@: %{public}@, but code "
                 "signing info has changed",
                 process.pid, process.binary.name, matchingRule);
  ...
  // no matching rule found?
    ...
    // dbg msg
    os_log_debug(logHandle, "no (saved) rule found for %d/%{public}@",
                 process.pid, process.binary.name);
  }
  ...
  if (YES == silentMode) {
    // entry
    NSMutableDictionary *pendingEntry =
        makeConnectionEntry((NEFilterSocketFlow *)flow, process, @"pending",
                            @"silent_mode_pending");

    // dbg msg
    os_log_debug(
        logHandle,
        "client in silent mode, queueing pending connection for review");
    ...
    // log allow event
    appendConnectionEvent(makeConnectionEntry(
        (NEFilterSocketFlow *)flow, process, @"allow", @"silent_mode_pending"));
  ...
  if (YES == [preferences.preferences[PREF_PASSIVE_MODE] boolValue]) {
    ...
    // user action: allow?
    if (PREF_PASSIVE_MODE_ALLOW ==
        [preferences.preferences[PREF_PASSIVE_MODE_ACTION] integerValue]) {
      // dbg msg
      os_log_debug(logHandle,
                   "passive mode: action is 'allow', so allowing %d/%{public}@",
                   process.pid, process.binary.name);
      ...
      // log
      appendConnectionEvent(makeConnectionEntry(
          (NEFilterSocketFlow *)flow, process, @"allow", @"passive_mode"));
    }
    ...
    // user action: block?
      ...
      // dbg msg
      os_log_debug(logHandle,
                   "passive mode: action is 'block', so blocking %d/%{public}@",
                   process.pid, process.binary.name);
      ...
      // log
      appendConnectionEvent(makeConnectionEntry(
          (NEFilterSocketFlow *)flow, process, @"block", @"passive_mode"));
    }

    // create rule?
    if (PREF_PASSIVE_MODE_RULES_YES ==
        [preferences.preferences[PREF_PASSIVE_MODE_RULES] integerValue]) {
      // dbg msg
      os_log_debug(logHandle, "passive mode: create rules is set, so creating "
                              "rule for new connection");

      // extract remote endpoint information
      NWHostEndpoint *remoteEndpoint =
          (NWHostEndpoint *)((NEFilterSocketFlow *)flow).remoteEndpoint;

      // init info for rule creation with specific endpoint information
      info = [@{KEY_PATH : process.path, KEY_TYPE : @RULE_TYPE_PASSIVE}
          mutableCopy];

      // get best hostname (prioritizes domain names over IP addresses)
      NSString *bestHostname =
          [self getBestHostnameFromFlow:(NEFilterSocketFlow *)flow];
      ...
      if (((NEFilterSocketFlow *)flow).socketProtocol > 0) {
        info[KEY_PROTOCOL] = [NSNumber
            numberWithInt:((NEFilterSocketFlow *)flow).socketProtocol];
      }
      ...
      if (YES != [rules add:[[Rule alloc] init:info] save:YES]) {
        // err msg
        os_log_error(logHandle,
                     "ERROR: failed to add (passive) rule for %{public}@",
                     info[KEY_PATH]);
  ...
  if (YES == [alerts isRelated:process]) {
    // dbg msg
    os_log_debug(logHandle,
                 "an alert is shown for process %d/%{public}@, so holding off "
                 "delivering for now...",
                 process.pid, process.binary.name);
  ...
  //   b) There are other rules for this same process (even though they didn't
  //   match)
  if ((YES == [preferences.preferences[PREF_ALLOW_APPLE] boolValue]) &&
    ...
    os_log_debug(
        logHandle,
        "'Allow Apple' preference is set, will check if is an Apple binary");
    ...
    if (Apple == [process.csInfo[KEY_CS_SIGNER] intValue]) {
      ...
      if (YES == [self.grayList isGrayListed:process]) {
        // dbg msg
        os_log_debug(logHandle,
                     "while signed by apple, %d/%{public}@ is gray listed, so "
                     "will alert",
                     process.pid, process.binary.name);
        ...
        [self alert:(NEFilterSocketFlow *)flow
             process:process
            csChange:csChange];
      }
      // other rules for this process?
        ...
        // dbg msg
        os_log_debug(logHandle,
                     "while signed by apple, %d/%{public}@ has other "
                     "(non-matching) rules, so will alert",
                     process.pid, process.binary.name);
      ...
      //  not on graylist and w/ no other rules, so allow
        ...
        // dbg msg
        os_log_debug(logHandle,
                     "due to preferences, allowing (non-graylisted) apple "
                     "process %d/%{public}@",
  ...
  // dbg msg
    ...
    os_log_debug(
        logHandle,
        "'Allow Apple' preference not set, so skipped 'Is Apple' check");
  }

  //'allow installed' check
  // if preference is enabled, item is 3rd-party, internal, and hasn't had its
  // CS changed ...allow!
  if ((YES == [preferences.preferences[PREF_ALLOW_INSTALLED] boolValue]) &&
    ...
    if (YES == isInternalProcess(process.path)) {
      ...
      os_log_debug(
          logHandle,
          "3rd-party (internal) app, plus 'PREF_ALLOW_INSTALLED' is set...");
      ...
      if ((nil != date) && (NSOrderedAscending == [date compare:installDate])) {
        // dbg msg
        os_log_debug(logHandle,
                     "3rd-party item was installed prior (%@) to LuLu (%@), "
                     "allowing & adding rule",
                     date, installDate);
        ...
        if (YES != [rules add:[[Rule alloc] init:info] save:YES]) {
          // err msg
          os_log_error(logHandle, "ERROR: failed to add rule for %{public}@",
                       info[KEY_PATH]);
      ...
      // newer
        ...
        os_log_debug(
            logHandle,
            "3rd-party item date (%@), is after LuLu's install date (%@)", date,
            installDate);
      }
    }
    // item is external
      ...
      os_log_debug(
          logHandle,
          "%{public}@ is external, so skipping 'allow installed' check",
          process.path);
    }
  }
  ...
  if ((YES == [preferences.preferences[PREF_ALLOW_DNS] boolValue]) &&
    ...
    // dbg msg
    os_log_debug(logHandle,
                 "'allow DNS traffic' is enabled, so checking port/protocol");

    // check proto (UDP) and port (53)
    if ((IPPROTO_UDP == ((NEFilterSocketFlow *)flow).socketProtocol) &&
        (YES ==
         [((NWHostEndpoint *)((NEFilterSocketFlow *)flow).remoteEndpoint).port
             isEqualToString:@"53"])) {
      // dbg msg
      os_log_debug(logHandle, "protocol is 'UDP' and port is '53', (so likely "
                              "DNS traffic) ...will allow");
      ...
      // log
      appendConnectionEvent(makeConnectionEntry(
          (NEFilterSocketFlow *)flow, process, @"allow", @"allow_dns"));
  ...
  if ((YES == [preferences.preferences[PREF_ALLOW_SIMULATOR] boolValue]) &&
    ...
    // dbg msg
    os_log_debug(logHandle,
                 "'allow simulator apps' is enabled, so checking process");

    // is simulator app?
    if (YES == isSimulatorApp(process.path)) {
      // dbg msg
      os_log_debug(logHandle, "%{public}@, is an simulator app, so will allow",
                   process.path);
      ...
      // log
      appendConnectionEvent(makeConnectionEntry(
          (NEFilterSocketFlow *)flow, process, @"allow", @"allow_simulator"));
  ...
  if ((nil == consoleUser) || (nil == alerts.xpcUserClient)) {
    // silent or strict mode?
    //  allow and queue for later review
    if ((YES == silentMode) || (YES == strictMode)) {
      // entry
      NSMutableDictionary *pendingEntry = makeConnectionEntry(
          (NEFilterSocketFlow *)flow, process, @"pending", @"no_user_pending");

      // dbg msg
      os_log_debug(logHandle, "no active user/client, queueing pending "
                              "connection for later review");

      // queue and log
      appendPendingConnection(pendingEntry);
      appendConnectionEvent(makeConnectionEntry(
          (NEFilterSocketFlow *)flow, process, @"allow", @"no_user_pending"));
    ...
    // dbg msg
    os_log_debug(logHandle, "no active user or no connected client, will allow "
                            "(and create rule)...");
...
bail:
  ...
  //  log stream --level debug --predicate 'subsystem ==
  //  "com.objective-see.lulu" && composedMessage BEGINSWITH "[LULU]"'
  os_log_debug(logHandle,
               "[LULU] PROCESS: %{public}@, FLOW (endpoint): %{public}@, RULE: "
               "%{public}@, verdict: %{public}@",
               process.path, ((NEFilterSocketFlow *)flow).remoteEndpoint,
               matchingRule, verdict);
...
- (void)alert:(NEFilterSocketFlow *)flow
      process:(Process *)process
     csChange:(BOOL)csChange {
  // alert
      ...
      [alerts
          deliver:alert
            reply:^(NSDictionary *alert) {
              ...
              os_log(logHandle,
                     "(user) response: \"%@\" for %{public}@, that was trying "
                     "to connect to %{public}@:%{public}@",
                     (RULE_STATE_BLOCK == [alert[KEY_ACTION] unsignedIntValue])
                         ? @"block"
                         : @"allow",
                     alert[KEY_PATH], alert[KEY_ENDPOINT_ADDR],
                     alert[KEY_ENDPOINT_PORT]);
...
- (void)addRelatedFlow:(NSString *)key flow:(NEFilterSocketFlow *)flow {
  // dbg msg
  os_log_debug(logHandle, "adding flow to 'related': %{public}@ / %{public}@",
               key, flow);
...
- (void)processRelatedFlow:(NSString *)key {
  ...
  // dbg msg
  os_log_debug(logHandle, "processing %lu related flow(s) for %{public}@",
               (unsigned long)[self.relatedFlows[key] count], key);
  ...
  @synchronized(self.relatedFlows) {
    ...
    for (NSInteger i = flows.count - 1; i >= 0; i--) {
      ...
      //  ...so stop, and wait for user response (which will retrigger
      //  processing)
...
- (Process *)createProcess:(NEFilterFlow *)flow {
  ...
  if (nil == process) {
    // err msg
    os_log_error(logHandle, "ERROR: failed to create process for %d",
...
- (NSString *)getBestHostnameFromFlow:(NEFilterSocketFlow *)flow {
  ...
  if (flow.URL.host.length) {
    // dbg msg
    os_log_debug(logHandle, "using flow.URL.host as best hostname: %{public}@",
                 flow.URL.host);
  ...
  if (@available(macOS 11, *)) {
    if (flow.remoteHostname.length) {
      // dbg msg
      os_log_debug(logHandle,
                   "using flow.remoteHostname as best hostname: %{public}@",
                   flow.remoteHostname);
  ...
  if (remoteEndpoint.hostname.length) {
    ...
    os_log_debug(
        logHandle,
        "using remoteEndpoint.hostname as fallback hostname: %{public}@",
