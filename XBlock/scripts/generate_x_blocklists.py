...
def fetch_announced_prefixes(asn: int, timeout: float) -> set[str]:
    ...
    try:
        request = Request(url, headers={"User-Agent": "XBlock/0.1"})
...
def render_nginx_robot_map(tokens: list[str]) -> str:
    lines = [
        ...
        "map $http_user_agent $block_x_robot {",
        "    default 0;",
    ]
    for token in tokens:
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Generate X blocklists for DNS and browser extensions."
    )
    parser.add_argument(
        "--asn-file",
        type=Path,
    ...
    prefix_cache = args.output_dir / "x_prefixes.txt"
    ...
    write_list(args.output_dir / "x_asns.txt", [f"AS{asn}" for asn in asns])
    write_list(args.output_dir / "x_prefixes.txt", prefix_strings)
    write_list(args.output_dir / "x_ipv4_prefixes.txt", ipv4_prefixes)
    write_list(args.output_dir / "x_ipv6_prefixes.txt", ipv6_prefixes)
    write_list(args.output_dir / "x_domains.txt", domains)
    write_list(args.output_dir / "x_robot_user_agents.txt", robot_tokens)

    (args.output_dir / "dnsmasq-x-block.conf").write_text(
        render_dnsmasq(domains), encoding="utf-8"
    )
    (args.output_dir / "unbound-x-block.conf").write_text(
        render_unbound(domains), encoding="utf-8"
    )
    (args.output_dir / "nginx-x-robots.conf").write_text(
    ...
    write_json(args.output_dir / "xdata.json", xdata)
